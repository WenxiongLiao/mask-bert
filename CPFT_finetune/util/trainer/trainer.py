...
class Trainer:
    def __init__(self, task, model,
                 ...
                 lr: float = 1e-4, betas=(0.9, 0.999), weight_decay: float = 0.01, warmup_steps=1000,epoch: int = 150,
        ...
        self.scheduler = get_cosine_schedule_with_warmup(
            self.optim,
            num_warmup_steps= int(0.1 * epoch * len(train_dataloader)),
    ...
    def train(self, epoch,fine_tune_model_path,base_num_labels):
        self.iteration(epoch, self.train_data,fine_tune_model_path,base_num_labels)

    def iteration(self, epoch, data_loader,fine_tune_model_path,base_num_labels, train=True):
        ...
        for i, data in data_iter:
            ...
            output = self.model.forward(**data, mode='train',device = self.device)
        ...
        # eval
        eval_acc = self.evaluation(epoch, self.val_data,base_num_labels, train=False)
        self.model.train
        # save
        ...
        if eval_acc > self.max_acc:
            ...
            self.model.model.save_pretrained(fine_tune_model_path)
            self.max_acc = eval_acc
            # self.max_acc_model_path = output_path
            self.count_for_stop = 0
            # torch.save(self.model.state_dict(), output_path)
            print("EP:%d Model Saved on:" % epoch, fine_tune_model_path)
            
        # else:
        #     self.count_for_stop += 1
        #     if self.count_for_stop == 10:
        #         # self.model.load_state_dict(torch.load(self.max_acc_model_path))
        #         self.model.model = BertForSequenceClassification.from_pretrained(fine_tune_model_path)
        #         self.model.to(self.device)
        #         eval_acc = self.evaluation(epoch, self.test_data,base_num_labels, train=False)
...
################################
    def evaluation(self, epoch, data_loader,base_num_labels, train=False):
        str_code = "train" if train else "test"
        ...
        with torch.no_grad():
            for i, data in data_iter:
                ...
                output = self.model.forward(**data, mode='test',device = self.device)
                predict = F.softmax(output.logits, dim=1).argmax(dim=1)
                correct = (predict == data['labels'].long() - base_num_labels).sum().item()
